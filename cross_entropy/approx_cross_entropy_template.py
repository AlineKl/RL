...
def rl_approx_cross_entropy(nn_agent):
    ...
    for i in range(total_iterations):
        ...
        nn_agent.partial_fit(elite_states, elite_actions)
...
def test_rl_approx_cross_entropy(nn_agent):
    ...
    for t in range(1000):
        ...
        a = np.random.choice(n_actions, p=nn_agent.predict_proba([s])[0])
...
if __name__ == '__main__':
    ...
    agent = MLPClassifier(activation='tanh', hidden_layer_sizes=(10, 10, ))
    ...
    for e in range(30):
        print('Episode %i:' % e)
        test_rewards.append(test_rl_approx_cross_entropy(agent))
    print('Mean test reward = %.3f' % np.mean(test_rewards))
