...
def update_policy(elite_states, elite_actions, n_states, n_actions):
    ...
    for e_s, e_a in zip(elite_states, elite_actions):
        new_policy[e_s][e_a] += 1

    r_sums = new_policy.sum(axis=1)
    for s in range(n_states):
        new_policy[s] = new_policy[s] / r_sums[s] if r_sums[s] != 0 else np.full(n_actions, 1./n_actions)
...
def rl_cross_entropy():
    ...
    percentile = 30  # take this percentage of 'elite' states/actions
    alpha = 0.2  # alpha-blending for policy updates
