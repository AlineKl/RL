...
def get_action_value(mdp, state_values, state, action, gamma):
    ...
    for s, prob_s in mdp.get_next_states(state, action).items():
        q += (mdp.get_reward(state, action, s) + gamma * state_values[s]) * prob_s
...
def get_new_state_value(mdp, state_values, state, gamma):
    ...
    values = [get_action_value(mdp, state_values, state, action, gamma) for action in possible_actions]
...
def get_optimal_action(mdp, state_values, state, gamma=0.9):
    ...
    possible_actions = mdp.get_possible_actions(state)
    q = {action: get_action_value(mdp, state_values, state, action, gamma) for action in possible_actions}
